...
if ($_GET["type"]=='')
 {
  ...
  print '<tr><td align="center" bgcolor="#ffffff" valign="middle" colspan="4">
	 <table><tr><td width="1200px">
	  ...
	  print '<tr class="BlockHeaderLeftRight" align="center"><td rowspan="2">Стало</td>';
	  print '<td colspan="2">Пиковые нагрузки нового оборудования</td>
		 ...
		 while ($ui3 = mysql_fetch_row ($e3))
			{
			 for ($tn=1; $tn<=$qnt; $tn++)
			    {
			     //if ($data_ven[$ccn][$tn]-$data_work[$ccn][$tn]>=$data_ven[$ccn][$tn]*0.05) 
	        	     //$base1[$tn]+=$data_ven[$ccn][$tn]*($welecs[$ccn]/$counts)*$tarif_elec[$tn]; // !!!!!!!!!!!!!!!!!!!!!!
			     //else 
			     $base1[$tn]+=$data_work[$ccn][$tn]*($welecs[$ccn]/$counts)*$tarif_elec[$tn]; // !!!! was until 27-05-2013

			//     if ($data_gor[$ccn][$tn]-$data_work[$ccn][$tn]>=$data_gor[$ccn][$tn]*0.05) 
			//        {

    		    	         //$base2[$tn]+=$data_gor[$ccn][$tn]*($gazs[$ccn]/$counts)*$tarif_gas[$tn];
			         //if ($ui3[25]!=8888) $base3[$tn]+=$data_gor[$ccn][$tn]*($pars[$ccn]/$counts)*$tarif_par[$tn];
			         //else $base3[$tn]+=$data_gor[$ccn][$tn]*($pars[$ccn]/$counts)*$tarif_par[$tn]/2;  // dush2

			//	}
			     //else
			        {
    		    	         $base2[$tn]+=$data_work[$ccn][$tn]*($gazs[$ccn]/$counts)*$tarif_gas[$tn]; // !!!! was until 27-05-2013
		 ...
		 while ($ui3 = mysql_fetch_row ($e3))
			{
			 ...
			 print '<td style="white-space:nowrap">'.$ui3[1].'</td><td class="simple">'.number_format($gas,2).'</td><td class="simple">'.number_format($elec,2).'</td>';
			 ...
			 if (!$_GET["hide"])
			    {
			     for ($tn=1; $tn<=$qnt; $tn++)
				if ($data_work[$ccn][$tn]>=$data_gor[$ccn][$tn]) { print '<td class="simple" align="center"><input id="'.$ccn.'-'.$tn.'-2-0" name="'.$ccn.'-'.$tn.'-2-0" class="simple2" value="'.number_format($data_gor[$ccn][$tn]-$data_work[$ccn][$tn],3).'" align="center"></td>'; $data_raz[$ccn][$tn]=$data_work[$ccn][$tn]-$data_gor[$ccn][$tn]; } 
				else 
				    {
				     if ($data_gor[$ccn][$tn]-$data_work[$ccn][$tn]>=0)
    				        { print '<td class="simple" align="center"><input style="background-color:pink" id="'.$ccn.'-'.$tn.'-2-0" name="'.$ccn.'-'.$tn.'-2-0" class="simple2" value="'.number_format($data_gor[$ccn][$tn]-$data_work[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>'; $data_raz[$ccn][$tn]=$data_gor[$ccn][$tn]-$data_work[$ccn][$tn]; }
				     else { $data_raz[$ccn][$tn]=$data_work[$ccn][$tn]-$data_gor[$ccn][$tn]; print '<td class="simple" align="center">'.$data_work[$ccn][$tn].'-'.$data_gor[$ccn][$tn].'<input id="'.$ccn.'-'.$tn.'-2-0" name="'.$ccn.'-'.$tn.'-2-0" class="simple2" value="'.number_format($data_raz[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>';  }
				    }
			     for ($tn=1; $tn<=$qnt; $tn++)
				if ($data_work[$ccn][$tn]>=$data_ven[$ccn][$tn]) { print '<td class="simple" align="center"><input id="'.$ccn.'-'.$tn.'-2-1" name="'.$ccn.'-'.$tn.'-2-1" class="simple2" value="'.number_format($data_ven[$ccn][$tn]-$data_work[$ccn][$tn],3).'" align="center"></td>'; $data_raz2[$ccn][$tn]=$data_work[$ccn][$tn]-$data_ven[$ccn][$tn]; }
				else 
				    {
				     if ($data_ven[$ccn][$tn]-$data_work[$ccn][$tn]>0) { print '<td class="simple" align="center"><input style="background-color:pink" id="'.$ccn.'-'.$tn.'-2-1" name="'.$ccn.'-'.$tn.'-2-1" class="simple2" value="'.number_format($data_ven[$ccn][$tn]-$data_work[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>'; $data_raz2[$ccn][$tn]=$data_ven[$ccn][$tn]-$data_work[$ccn][$tn]; }
				     else { $data_raz2[$ccn][$tn]=$data_work[$ccn][$tn]-$data_ven[$ccn][$tn]; print '<td class="simple" align="center"><input id="'.$ccn.'-'.$tn.'-2-0" name="'.$ccn.'-'.$tn.'-2-0" class="simple2" value="'.number_format($data_raz2[$ccn][$tn],3).'" align="center" style="background:#dedede"></td>';  }
				    }
...
//if (1)
				 //$w=$w-($w/$data_ven[$ccn][$tn])*$data_raz2[$ccn][$tn];
				 //$g=$g-($g/$data_ven[$ccn][$tn])*$data_raz[$ccn][$tn];
//				 if ($ui2[17])
				 if ($ui2[18]!=28) 
				 //if (1)
				    {
  ...
  print '<tr class="BlockHeaderLeftRight" align="center"><td rowspan="3">Прочие расходы</td>
	 <td class="simple" colspan="'.($qqq*$qnt+6).'" rowspan="3"></td>';
  ...
  print '<tr class="BlockHeaderLeftRight" align="center"><td colspan="1"></td>
  ...
  for ($tn=1; $tn<=($qnt*$qqq+7); $tn++) print '<td align="center" class="simple"></td>';
  for ($tn=1; $tn<=$qnt; $tn++)
	 print '<td colspan="4" class="simple"></td><td>Итого:</td><td>'.number_format($sum_ek[$tn],2).'</td>';
  print '</tr>';
  print '<tr class="BlockHeaderLeftRight" align="center">';
  print '<td align="center" class="simple" colspan="'.($qnt*$qqq+7).'"></td>';
  for ($tn=1; $tn<=$qnt; $tn++) { print '<td class="simple"></td><td colspan="4">Итого к оплате без НДС (80%):</td><td>'.(number_format($sum_ek[$tn]*0.8,2)).'</td>'; $sss1+=$sum_ek[$tn]*0.8; }
  print '</tr>';
  print '<tr class="BlockHeaderLeftRight" align="center">';
  print '<td align="center" class="simple" colspan="'.($qnt*$qqq+7).'"></td>';
  for ($tn=1; $tn<=$qnt; $tn++) { print '<td class="simple"></td><td colspan="4">Итого к оплате с НДС</td><td>'.(number_format(($sum_ek[$tn]*0.8)*1.18,2)).'</td>'; $sss2+=($sum_ek[$tn]*0.8)*1.18; }
  print '</tr>';
  print '<tr class="BlockHeaderLeftRight" align="center">';
  print '<td align="center" class="simple" colspan="'.($qnt*$qqq+7).'"></td>';
  print '<td align="center" class="simple">(b)'.$sss1.'</td>';
  print '<td align="center" class="simple">(a)'.$sss2.'</td>';
  print '</form>';
  print '</table>';
 }
?>