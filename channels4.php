		 ...
		 while ($uo = mysql_fetch_row ($e3))
			{
			 ...
			 if ($uo[9]==4)
				{
    				 $query = 'SELECT MAX(value),date FROM hours WHERE type=1 AND value<150 AND device='.$device.' AND channel='.$uo[0];
    				 if ($e4 = mysql_query ($query,$i))
				 if ($uo2 = mysql_fetch_row ($e4))
					{ $pvalue1=$uo2[0]; $ptime1=$uo2[1]; } 
    				 $query = 'SELECT MIN(value),date FROM hours WHERE type=1 AND value>0 AND device='.$device.' AND channel='.$uo[0];
    				 if ($e4 = mysql_query ($query,$i))
				 if ($uo2 = mysql_fetch_row ($e4))
